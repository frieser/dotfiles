...
Panel {
    ...
    // Highlight state for power changes
    ...
    // Timer to remove power change highlight
    Timer {
        id: powerChangeTimer
        interval: 2000
        onTriggered: {
            root.powerChangeHighlight = false;
        }
    }
    ...
    ColumnLayout {
        ...
        BatteryIndicator {
            ...
            onPowerStateChanged: {
                // Show panel when power state changes
                root.revealed = true;
                root.powerChangeHighlight = true;
                statusHideTimer.restart();
